...
class UserBehaviorServiceImplTest {
    ...
    @Test
    void recordEvent_shouldSetIdAfterInsert() {
        UserBehaviorEvent event = new UserBehaviorEvent();
        event.setUserId(1L);
        event.setProductId(10L);
        event.setAction("CLICK");

        doNothing().when(behaviorEventMapper).insert(any(UserBehaviorEvent.class));
    ...
    @Test
    void computeFeatureMatrix_shouldCountActionsCorrectly() {
        ...
        doNothing().when(featureMatrixMapper).insert(any(UserFeatureMatrix.class));
